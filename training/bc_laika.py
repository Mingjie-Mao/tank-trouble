...
def collect(n_samples, seed0=3_000_000):
    """跑 Laika 镜像局, 从 tank0 视角记录 (观测, 动作) 对。

    动作标签 = 该决策步(2帧)结束时 tank0 的输入状态 (Laika 的动作
    多为持续数帧的驾驶/转向指令, 帧内标签噪声很小)。
    """
    env = TankTroubleGym(seed=seed0, obs_traj=True)
    X = np.zeros((n_samples, env.observation_space.shape[0]), dtype=np.float32)
...
def train_bc(X, Y, epochs=12, batch=4096, lr=3e-4, val_frac=0.03):
    """在新建 PPO 的策略网络上做监督 BC; 返回 PPO 模型 (价值头未训)。"""
    from stable_baselines3 import PPO

    env = TankTroubleGym(seed=0, obs_traj=True)
...
def main():
    ...
    model = train_bc(X, Y, epochs=args.epochs)
    model.save(args.out)
